...
function App(){
 ...
 const switchLang=(e)=>{e.preventDefault();const next=lang==='zh'?'en':'zh';setLang(next);history.pushState({},'',`/${next}`);document.documentElement.lang=next==='zh'?'zh-CN':'en';document.title=next==='zh'?'李开智 · 高级前端 / AI 全栈 & Web3 工程师':'Kaizhi Li · Senior Frontend / AI Full-stack & Web3 Engineer'};
 React.useEffect(()=>{document.documentElement.lang=lang==='zh'?'zh-CN':'en';document.title=lang==='zh'?'李开智 · 高级前端 / AI 全栈 & Web3 工程师':'Kaizhi Li · Senior Frontend / AI Full-stack & Web3 Engineer';const fn=()=>setLang(escPath()); addEventListener('popstate',fn); return()=>removeEventListener('popstate',fn)},[]);
 return <main className="page"><nav className="no-print"><a href={lang==='zh'?'/en':'/zh'} onClick={switchLang}>{d.navLanguage}</a><button onClick={()=>window.print()}>{d.print}</button></nav>
  ...
  <Section title={d.sections.projects} label={d.sectionLabels.projects}><div className="cards">{d.projects.map((p,i)=>{const [name,role,desc]=Array.isArray(p)?p:[p.name,p.role,p.desc];return <article key={i}><h3>{name}</h3><small>{role}</small><p>{desc}</p>{p.bullets&&<ul>{p.bullets.map(x=><li key={x}>{x}</li>)}</ul>}</article>})}</div></Section>
  <Section title={d.sections.side} label={d.sectionLabels.side}><div className="cards">{d.side.map((p,i)=><article key={i}><h3><Link href={p.link} externalArrow>{p.name}</Link></h3><small>{p.tag}</small><p>{p.desc}</p>{p.bullets&&<ul>{p.bullets.map(x=><li key={x}>{x}</li>)}</ul>}<div className="tech">{d.techLabel} · {p.tech}</div></article>)}</div></Section>
  <Section title={d.sections.open} label={d.sectionLabels.open}><div className="open-grid">{d.open.map(([name,desc,href])=><a href={href} target="_blank" rel="noreferrer" key={name}><strong>{name} ↗</strong><span>{desc}</span></a>)}</div></Section>
