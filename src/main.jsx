...
function App(){
 ...
 return <main className="page"><nav className="no-print"><a href={lang==='zh'?'/en':'/zh'} onClick={switchLang}>{d.navLanguage}</a><button onClick={()=>window.print()}>{d.print}</button></nav>
  ...
  <Section title={d.sections.projects} label={d.sectionLabels.projects}><div className="cards">{d.projects.map((p,i)=>{const [name,role,desc]=Array.isArray(p)?p:[p.name,p.role,p.desc];return <article key={i}><div className="project-head"><div><h3>{name}</h3><small>{role}</small></div>{p.company&&<span className="project-company">{p.company}</span>}</div><p>{desc}</p>{p.bullets&&<ul>{p.bullets.map(x=><li key={x}>{x}</li>)}</ul>}{p.tech&&<div className="tech">{d.techLabel} · {p.tech}</div>}</article>})}</div></Section>
